...
class RonnyaDB:
    ...
    async def get_info(self, fid: str) -> int:
        '''
        Args
        ...
        try:
            ...
            if len(result) == 0:
                ...
                self.cur.execute(query,(fid,userdata['account_id'],userdata['nickname'],userdata['level']['id'],userdata['level']['score'],userdata['level3']['id'],userdata['level3']['score']))
                self.session.commit()
                ...
                if diff <= 600: #업데이트한지 10분 이상 지났을 경우 -> db 업데이트
                    ...
                    query = '\
                        ...
                        WHERE fid = %s'
                    self.cur.execute(query,(userdata['level']['id'],userdata['level']['score'],userdata['level3']['id'],userdata['level3']['score'],datetime.now(),fid))
                    self.session.commit()
                    return 1
